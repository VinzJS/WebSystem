...
class User{
	...
	public function new_user($email,$password,$lastname,$firstname){
		...
		$data = [
			[$lastname,$firstname,$email,$password,$NOW,$NOW,'1'],
		];
		$stmt = $this->conn->prepare("INSERT INTO tbl_users (user_lastname, user_firstname, user_email, user_password, user_date_added, user_time_added, user_status) VALUES (?,?,?,?,?,?,?)");
		try {
	...
	public function update_user($lastname,$firstname, $access, $id){
		...
		$sql = "UPDATE tbl_users SET user_firstname=:user_firstname,user_lastname=:user_lastname,user_date_updated=:user_date_updated,user_time_updated=:user_time_updated WHERE user_id=:user_id";
