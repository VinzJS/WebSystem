...
class User{
	...
	public function check_login($useremail, $password){
		$sql = "SELECT user_email, user_password FROM tbl_users WHERE user_email = :email"; 
		$q = $this->conn->prepare($sql);
		$q->execute(['email' => $useremail]);
	  ...
	  function get_user_info_logged_in() {
		global $user; // Add this line to access the $user variable in the function
	
		// Assuming the logged-in user's email is stored in the session as 'user_email'
		...
		return null; // Return null if the user email is not set in the session
	}
}